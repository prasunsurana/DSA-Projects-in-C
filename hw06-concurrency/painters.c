...
int main(){
	...
	//Null checks for the mallocs of each of the 4 artists
	if (Michaelangelo == NULL || Donatello == NULL || Raphael == NULL
		|| Leonardo == NULL){
		return 1;
	}
	...
	//Allocate memory for the array of rookie artist	
	artist_t* moreArtists = malloc(sizeof(artist_t)*rookieArtists);
	
	//Null check for the array of rookie artists
	if (moreArtists == NULL){
		return 1;
	}

	//Initialize the starting positions and the paint colour for
	//each rookie artist, as well as create a thread for each
	//individual artist
