...
queue_t* create_queue(unsigned int _capacity){
	...
	//Allocate memory to a new queue
	...
	myQueue->data = (graph_edge_t**)malloc(sizeof(graph_edge_t*)*myQueue->capacity);
...
graph_edge_t* queue_dequeue(queue_t *q){
	...
	//If the queue is empty, crash the program
	if (queue_empty(q) == 1){
		exit(1);
	}
	...
	for (int i = toRemove; i < q->back; i++){
        ...
        graph_edge_t* minEdge = q->data[toRemove];
