...
//Tests insertion of multiple keys in 
//the same bucket, and then removes
//a non-head node
int unitTest6(int status){
	int passed = 0;
	hashmap_t* test = hashmap_create(12);
	hashmap_insert(test, "cow", "A common farm animal.");
	hashmap_insert(test, "dog", "A common house pet.");
	hashmap_insert(test, "cat", "A common enemy.");
	hashmap_removeKey(test, "dog");

	if (hashmap_hasKey(test, "dog") == 0){
		passed = 1;
	}
...
//Tests insertion of multiple keys in
//different buckets, and then gets value
//of a particular key
int unitTest7(int status){
	int passed = 0;
	hashmap_t* test = hashmap_create(12);
	hashmap_insert(test, "cow", "A common farm animal.");
	hashmap_insert(test, "horse", "A racing animal.");
	hashmap_insert(test, "tiger", "A dangerous animal.");
	hashmap_insert(test, "globfish", "The ugliest fish in the world.");
	hashmap_update(test, "globfish", "Jk!");

	if (strcmp(hashmap_getValue(test, "globfish"),"Jk!") == 0){
		passed = 1;
	}
...
//Tests insertion of multiple identical
//key-value pairs, removes it and then
